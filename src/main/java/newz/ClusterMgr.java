...
public class ClusterMgr
{
   ...
   public void extrudeCluster(Cluster cluster, Point2D observer, double extrusionDistance)
   {
      ...
      if (cluster.getType() == Type.POLYGON)
      {
         ...
         extrudePolygon(cluster, extrusionDistance);
      }
   }
   ...
   private void extrudedFirstNonNavigableExtrusion(Cluster cluster, int index, double extrusionDistance)
   {
      ...
      Point2D normal1 = new Point2D(cluster.getListOfSafeNormals().get(cluster.getListOfSafeNormals().size() - 1).getX(), cluster.getListOfSafeNormals().get(cluster.getListOfSafeNormals().size() - 1).getY());
   ...
   private void extrudePolygon(Cluster cluster, double extrusionDistance)
   {
      extrusionDistance = extrusionDistance + cluster.getAdditionalExtrusionDistance();
      extrudedFirstNonNavigableExtrusion(cluster, extrusionIndex, extrusionDistance - 0.01);
      extrudedNonNavigableBoundary(extrusionIndex, cluster, extrusionDistance - 0.01);
      extrudedNavigableBoundary(extrusionIndex, cluster, extrusionDistance);
   }

   private ArrayList<Point2D> extrudeLine(Point2D pt1, Point2D pt2, double extrusion)
   {
      ...
      Point2D endExtrusion1 = new Point2D(pt2.getX() + vec21.getX() * extrusion, pt2.getY() + vec21.getY() * extrusion);
      vec21.negate();
      Point2D endExtrusion2 = new Point2D(pt1.getX() + vec21.getX() * extrusion, pt1.getY() + vec21.getY() * extrusion);

      Point2D midNormal1 = EuclidGeometryTools.perpendicularBisectorSegment2D(pt1, pt2, extrusion).get(0);
      Point2D midNormal2 = EuclidGeometryTools.perpendicularBisectorSegment2D(pt1, pt2, extrusion).get(1);

      points.add(endExtrusion2);
      points.add(extrudeCorner(pt1, vec21, endExtrusion2, midNormal1, extrusion));
      points.add(midNormal1);
      points.add(extrudeCorner(pt2, vec21, endExtrusion1, midNormal1, extrusion));
      points.add(endExtrusion1);
      points.add(extrudeCorner(pt2, vec21, endExtrusion1, midNormal2, extrusion));
      points.add(midNormal2);
      points.add(extrudeCorner(pt1, vec21, endExtrusion2, midNormal2, extrusion));
      points.add(endExtrusion2);
